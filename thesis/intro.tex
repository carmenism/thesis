...
\begin{align}
\frac{d N_i}{dt} &= \underbrace{r_i}_\text{Growth rate} N_i \left(\underbrace{1 - \underbrace{\frac{N_i}{K_G}}_\text{Carrying capacity}}_\text{Prevents infinite growth} - \underbrace{\frac{\displaystyle\sum\limits_{j=1}^g \beta_{ij} N_j}{K_G} - \frac{\displaystyle\sum\limits_{j=1}^G \beta_{ij} N_j}{K_{\sigma} - K_G}}_\text{Competition} \right) - \underbrace{N_i \displaystyle\sum\limits_{j=1}^P \alpha_{ij} N_j}_\text{Predation} - \underbrace{H_i N_i}_\text{Harvest}
\end{align}
