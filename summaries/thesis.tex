...
\begin{align}
   \frac{d x_1}{dt} &=   x_1(\alpha - \beta  x_2) 
\\ \frac{d x_2}{dt} &= - x_2(\gamma - \delta x_1)
\end{align}
...
The Lotka-Volterra model can be modified to take competition instead of predation into account, as in the Rosenzweig-MacArthur model \cite{Rosenzweig1963Graphical}.  This model also considers carrying capacity, which is the maximum number of a species that can sustained indefinitely:

\begin{align}
   \frac{d x_1}{dt} &= r_1 x_1 (1 - (\frac{x_1 + \alpha_{12} x_2}{K_1}))
\\ \frac{d x_2}{dt} &= r_2 x_2 (1 - (\frac{x_2 + \alpha_{21} x_1}{K_2}))
\end{align}
where $r_i$ is the growth rate for species $i$, $K_i$ is the carrying capacity for species $i$, and $\alpha_{ij}$ is the effect species $j$ has on species $i$.  As with Lotka-Volterra, Rosenzweig-MacArthur concerns only two species, but it can be generalized to include more than two.

Both Lotka-Volterra and Rosenzweig-MacArthur do not incorporate a factor which is critical when discussing fisheries management: the effect of harvest.  For this, the Schaefer model is most often used \cite{schaefer1957}:

\begin{align}
   \frac{d x}{dt} &= r x (1 - (\frac{x}{K})) - q E N
\end{align}
where $r$ is the growth rate, $K$ is the carrying capacity, $q$ is the catchability coefficient, and $E$ is the fishing effort.

Simple models are much appreciated when correct; since fewer components are needed to describe their real-world counterparts, they are more easily understood.  All three of these models are subjectively simple in that they only consider a few ecological factors each. However, a more holistic approach called ecosystem-based fishery management (EBFM) has been advocated \cite{united1999ecosystem}.  %Many models tend to leave out some ecological factors on the basis they do not contribute significantly to the decline of fish populations. 
